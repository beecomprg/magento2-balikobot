...
class Balikobot extends \Magento\Backend\App\Action
{
    /**
        ...
        try {
            ...
            if ($packages) {
                foreach ($packages as $shipper => $packageIds) {
                    $client->order($shipper, $packageIds);
                }
            }

            $this->messageManager->addSuccessMessage(__('Balikobot order complete. Car is on its way.'));
